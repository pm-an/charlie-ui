...
function AvatarGroup({ className, children, max, ...props }: AvatarGroupProps) {
  ...
  return (
    ...
    >
      ...
      {overflowCount > 0 && (
        <span className="relative inline-flex h-8 w-8 items-center justify-center rounded-full bg-bg-subtle-hover text-xs font-medium text-text-loud ring-2 ring-bg">
          +{overflowCount}
        </span>
      )}
    </div>
