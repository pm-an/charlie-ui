  ...
  return (
    <div data-slot="changelog-entry" className={cn("flex flex-col md:flex-row gap-4 md:gap-8", className)} {...props}>
      ...
      <div className="flex md:w-[120px] shrink-0 gap-3 items-center md:items-start">
        <time className="text-sm text-white/70" dateTime={date}>{date}</time>
        {version && (
          <span className="md:hidden inline-block rounded-md bg-white/5 px-2 py-0.5 text-xs text-white/70">
            {version}
          </span>
        )}
      </div>
      ...
      <div className="flex-1 pb-8 md:pb-12 border-b md:border-b-0 border-white/[0.06] last:border-b-0">
        {version && (
          <span className="mb-2 hidden md:inline-block rounded-md bg-white/5 px-2 py-0.5 text-xs text-white/70">
            {version}
          </span>
        )}
        ...
        {tags && tags.length > 0 && (
          <div className="mt-3 flex gap-2">
            {tags.map((tag) => (
              <span
                key={tag}
                className="inline-flex items-center rounded-md bg-white/5 px-2 py-0.5 text-xs text-white/70"
              >
                {tag}
              </span>
            ))}
